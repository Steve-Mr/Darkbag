...
class CameraFragment : Fragment() {
        ...
        imageAnalyzer = ImageAnalysis.Builder()
            ...
            .also {
                it.setAnalyzer(cameraExecutor, LuminosityAnalyzer { luma, iso, time ->
                    if (isFastExposureEnabled) {
                        // Use provided metadata or fallback to last known requested values
                        val effectiveIso = iso ?: currentIso
                        val effectiveTime = time ?: currentExposureTime
                        lifecycleScope.launch(Dispatchers.Main) {
                            updateClosedLoopExposure(luma, effectiveIso, effectiveTime)
                        }
                    }
                })
            }
    ...
    private class LuminosityAnalyzer(listener: LumaListener? = null) : ImageAnalysis.Analyzer {
        ...
        override fun analyze(image: ImageProxy) {
            ...
            // Note: Direct access to CameraFragment state (currentIso/Time) is not possible here
            // as this is a nested static class. Instead, we pass null if extraction fails,
            // and let the listener (which captures the Fragment scope) decide on fallback values.

            try {
                 // Attempt to extract metadata if available (e.g. via Camera2Interop)
                 // However, as the API is tricky, we rely on the listener's fallback strategy.
                 // If we had access to Camera2Interop here without build errors, we would use it.
            } catch (e: Exception) {
                // Ignore
            }

            // Call all listeners with new value (iso/time are null here, listener handles it)
            ...
            try {
                ...
                android.hardware.camera2.DngCreator(chars, captureResult).use { dngCreatorReal ->
                    ...
                    if (processedBitmap != null) {
                        ...
                        val scale = min(
                            maxThumbSize.toDouble() / processedBitmap.width,
                            maxThumbSize.toDouble() / processedBitmap.height
                        )
