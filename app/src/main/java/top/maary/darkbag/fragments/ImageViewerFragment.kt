...
open class ImageViewerFragment : Fragment() {
    ...
    private fun updateEffectsButtons() {
        ...
        val currentGroup = adapter.getGroup(binding.imagePager.currentItem)
        val isTB = isTBLayout(currentGroup, config)
    ...
    private fun updateSelectionFeedback() {
        val currentGroup = adapter.getGroup(binding.imagePager.currentItem)
        val isTB = isTBLayout(currentGroup, currentEditConfig)
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    if (!currentGroup.isHalfFrame()) {
                        ...
                        if (b1 != null || b2 != null) {
                            val isSBS = !isTBLayout(currentGroup, config)
    ...
    private fun isTBLayout(group: top.maary.darkbag.models.ImageGroup?, config: top.maary.darkbag.models.EditConfig? = null): Boolean {
        val layout = config?.hfLayout ?: group?.hfLayout
        return layout == "TB" || layout?.contains("top", ignoreCase = true) == true
    }
