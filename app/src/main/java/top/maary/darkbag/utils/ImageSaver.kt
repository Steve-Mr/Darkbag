...
object ImageSaver {
    ...
    suspend fun saveProcessedImage(
                        ...
                        if (isFastPath) {
                            ...
                            if (finalPath != null) {
                                ...
                                if (targetUri != null) {
                                    ...
                                    finalJpgUri = saveJpegToMediaStore(context, "$baseName.jpg", null, editConfig = editConfig, zoomFactor = zoomFactor, captureMetadata = captureMetadata, writeExifMetadata = !isFastPath, allowOverwriteByName = !isFastPath) { out ->
                                        finalFile.inputStream().use { it.copyTo(out) }
                                    }
                                }
        ...
        val dngJob = async(Dispatchers.IO) {
            ...
            if (dngFile.exists()) {
                val dngDisplayName = "$baseName.dng"
