...
extern "C" JNIEXPORT jint JNICALL
Java_top_maary_darkbag_processor_ColorProcessor_processZslBitmapWithLut(
        JNIEnv* env,
        jobject /* this */,
        jobject bitmap,
        jint targetLog,
    ...
    for (int i = 0; i < totalPixels; ++i) {
        uint32_t p = pixelData[i];
        float r = ((p >> 0) & 0xFF) / 255.0f; // RGBA_8888 memory layout: R is low byte
        float g = ((p >> 8) & 0xFF) / 255.0f;
        float b = ((p >> 16) & 0xFF) / 255.0f;

        Vec3 color = {r, g, b};
        // Simple sRGB to linear approximation is usually x^2.2, but here we keep it fast.
        // Direct application. Apply log then LUT.
        color = {apply_log(color.r, targetLog), apply_log(color.g, targetLog), apply_log(color.b, targetLog)};
        if (lut.size > 0) color = apply_lut(lut, color);

        int R = std::min(255, std::max(0, (int)(color.r * 255.0f)));
        int G = std::min(255, std::max(0, (int)(color.g * 255.0f)));
        int B = std::min(255, std::max(0, (int)(color.b * 255.0f)));

        pixelData[i] = (p & 0xFF000000) | (B << 16) | (G << 8) | R;
    }
