...
The library\x92s development is focused on:
...
Additionally, the LibRaw library offers some basic RAW conversion, intended for cases when such conversion is not the main function of the LibRaw-using application _(for example: a viewer for 500+ graphic file formats, including RAW)._ These methods are inherited from the Dave Coffin\x92s dcraw.c utility _(see below the \x93Project history\x94 section);_ their further development is not currently planned, because we do not consider production-quality rendering to be in the scope of LibRaw\x92s functionality _(the methods are retained for compatibility with prior versions and for rapid-fire testing of RAW support and other aspects)._
...
If you modify/add/improve the LibRaw source code, then your patches can only be included into the library\x92s official source code if you agree to it being distributed under both of the above licenses.
...
* Minor updates _(0.20.1, 0.20.2\x85)_ do not change the API/ABI and new cameras are not added; generally they are just bugfixes.
...
* Public snapshots are always tested on a fairly large user base, and may be considered suitable for use in programs that work with files with known origins (that is, recorded directly by users\x92 digital cameras). However, public snapshots should not be considered sufficiently reliable for processing files that are specially constructed for vulnerability testing; that is, they should not be used in public services that allow for anonymous processing of files of unknown provenance
...
LibRaw uses code from Dave Coffin\x92s dcraw.c utility (without RESTRICTED/GPL2 code):
...
LibRaw uses Roland Karlsson\x92s X3F tools source code, licensed under BSD license:
