...
**Learning:** High-frequency per-frame operations (like `ImageReader` / YUV preview frame analyzers) in Android are extremely sensitive to allocations. Using idiomatic Kotlin like `data.map { ... }.average()` on a 640x480 pixel array results in over 300,000 boxed `Integer` allocations per frame, leading to massive GC pressure (~140MB/s at 30fps).
